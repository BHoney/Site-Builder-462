...
//	public Texture2D texture = null;
	...
	private Texture2D cannyF ( ) {
		Texture2D b = LoadPNG("/Users/angelrodriguez/Desktop/bp1.jpg");
		int width = b.width;
		print ("W " + width);
	
		int height = b.height;
		print ("H" + height);
		Texture2D n = new Texture2D (width, height);
		...
		for ( int i = 0; i < width; i++ )
		{
			...
			{
				allPixR[i, j] = (int)b.GetPixel (i, j).r;
//				print ("bb " + allPixR [i, j]);
				allPixG[i, j] = (int)b.GetPixel (i, j).g;
				allPixB[i, j] = (int)b.GetPixel (i, j).b;
			}
		}
...
//				n.SetPixel (i, j, Color.FromArgb (red, green, blue));
				n.SetPixel (i, j, new Color(red, green, blue));
			}
		}
		...
		for ( int i = 0; i < width; i++ )
		{
			for ( int j = 0; j < height; j++ )
			{
				allPixRn[i, j] = (int)n.GetPixel (i, j).r;
				allPixGn[i, j] = (int)n.GetPixel (i, j).g;
				allPixBn[i, j] = (int)n.GetPixel (i, j).b;
			}
		}
		...
		// Bitmap bb = new Bitmap (pictureBox1.Image);
		Texture2D bb = new Texture2D(width,height);            
...
//		pictureBox2.Image = bb;
	...
	void OnGUI() {
		Texture2D texture = cannyF ();
		Texture2D b = LoadPNG("/Users/angelrodriguez/Desktop/bp1.jpg");

		//		(new Rect (20, 20, texture.width/2, texture.height/2)

		Rect rect = new Rect(Screen.width/2, 200, texture.width/2, texture.height/2);//Rect to put the texture on
		Rect rect2 = new Rect(Screen.width/12, 200, texture.width/2, texture.height/2);//Rect to put the texture on
		if (texture != null) {
			GUI.DrawTexture(rect, texture,ScaleMode.StretchToFill);//draws the texture on the rect
			GUI.DrawTexture(rect2, b,ScaleMode.StretchToFill);//draws the texture on the rect
